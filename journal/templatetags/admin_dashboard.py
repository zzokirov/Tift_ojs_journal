...
def get_chart_data():
    ...
    # Country stats
    country_data = Article.objects.exclude(author__country__isnull=True)\
        .exclude(author__country='')\
        .values('author__country')\
        .annotate(count=Count('id'))\
        .order_by('-count')[:5]
    country_labels = [c['author__country'] for c in country_data] if country_data else []
    country_counts = [c['count'] for c in country_data] if country_data else []

    # Gender stats
    gender_data = Article.objects.exclude(author__gender__isnull=True)\
        .exclude(author__gender='')\
        .values('author__gender')\
        .annotate(count=Count('id'))
    gender_map = {'male': 'Erkak', 'female': 'Ayol'}
    gender_labels = [gender_map.get(g['author__gender'], g['author__gender']) for g in gender_data] if gender_data else []
    gender_counts = [g['count'] for g in gender_data] if gender_data else []
    ...
    return json.dumps({
        ...
        'country': {'labels': country_labels, 'data': country_counts},
